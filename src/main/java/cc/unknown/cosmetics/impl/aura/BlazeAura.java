...
public class BlazeAura extends CosmeticBase {
	BlazeAura.BlazeModel blazeModel;

	public BlazeAura(RenderPlayer player) {
		super(player);
		this.blazeModel = new BlazeAura.BlazeModel(player);
	}

	@Override
	public void render(AbstractClientPlayer player, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		if (CosmeticController.shouldRenderCosmeticForPlayer(player, CosmeticType.AURA, "Blaze") && !player.isInvisible() && mc.thePlayer == player) {
