...
public class Cape implements LayerRenderer<AbstractClientPlayer>, Accessor {	
	...
	public void doRenderLayer(AbstractClientPlayer entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		...
		if (cosmetics.isEnabled() && cosmetics != null && entitylivingbaseIn.hasPlayerInfo() && !entitylivingbaseIn.isInvisible() && entitylivingbaseIn.isWearing(EnumPlayerModelParts.CAPE) && entitylivingbaseIn == mc.thePlayer) {
	        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
	        
	        CapeType selectedCape = cosmetics.capeType.getMode(CapeType.class);
