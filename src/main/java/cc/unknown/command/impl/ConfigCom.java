...
public final class ConfigCom extends Command {
	...
	public void execute(final String[] args) {
	    ...
	    if (args.length == 2) {
	        ...
	        switch (command) {
	        case "load":
	            if (Haru.cfgMngr.load(cfg)) {
	                Haru.cfgMngr.currentConfig = name;
	                success("Config loaded: " + name);
	            } else {
	                warning("Unable to load the config: " + name);
	            }
	            break;
	        case "guardar":
	        case "save":
	        ...
	        case "create":
	            try {
	                ...
	                error("Unable to create the config: " + name);
	            }
	            break;
	        case "remove":
	        case "delete":
	        ...
	        case "borrar":
	            if (configFile.exists()) {
	                if (configFile.delete()) {
	                    ...
	                    warning("Unable to delete the config: " + name);
	                }
	            } else {
	...
	private void handleSave(String configName) {
	    if (Haru.cfgMngr.save(new Config(configName))) {
	        success("Config saved: " + configName);
	    } else {
	        error("Error saving the config: " + configName);
	    }
	}
